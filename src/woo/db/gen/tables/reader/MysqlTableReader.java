...
{
  private static final String GET_COLUMN = " SELECT upper(COLUMN_NAME) as COLUMN_NAME,        DATA_TYPE,        NUMERIC_SCALE,        (CASE COLUMN_KEY WHEN 'PRI' THEN 'Y' ELSE 'N' END) AS PK,        (CASE EXTRA WHEN 'auto_increment' THEN 'Y' ELSE 'N' END) AS AI  FROM COLUMNS  WHERE TABLE_NAME = ? AND TABLE_SCHEMA = ? ";
  ...
  private TableValue getTable(Connection conn, String dbName, String tableName)
    throws Exception
  {
    PreparedStatement ps = conn.prepareStatement(" SELECT upper(COLUMN_NAME) as COLUMN_NAME,        DATA_TYPE,        NUMERIC_SCALE,        (CASE COLUMN_KEY WHEN 'PRI' THEN 'Y' ELSE 'N' END) AS PK,        (CASE EXTRA WHEN 'auto_increment' THEN 'Y' ELSE 'N' END) AS AI  FROM COLUMNS  WHERE TABLE_NAME = ? AND TABLE_SCHEMA = ? ");
